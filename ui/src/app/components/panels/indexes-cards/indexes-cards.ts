...
@Component({
  ...
  imports: [...MATERIAL_IMPORTS, GeneralIndexCard],
...
export class IndexesCards implements OnInit {
  ...
  ngOnInit(): void {
    const indexesList = this.INDEXES_NAME.map((row) => row.id);
    this.liveDataService
      .getIndexesData(indexesList)
      .pipe(
        // retry(3),
        catchError((error) => {
      ...
      .subscribe({
        next: (response: IndexCardInterface[]) => {
          response.map((item1) => {
            const match = this.INDEXES_NAME.find(
