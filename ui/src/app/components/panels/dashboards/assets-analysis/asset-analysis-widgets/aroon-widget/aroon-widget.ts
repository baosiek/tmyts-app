...
export class AroonWidget {
  ...
  getIndicatorData(){
    this.indicatorService.getAroonIndicator([this.resolvedData().symbol])
      .pipe(
        catchError<any, any>(
          (error: { error: { detail: any; }; }) => {
  ...
  dataIntoChartDataStructure(chartData: IndicatorModel[]) {
    ...
    for (const dataPoint of chartData) {
      ...
      this.aroon_up.push(
        [
          Number(dataPoint.date),
          Number(dataPoint.indicator['aroon_up'])
        ]
      );
      this.aroon_down.push(
        [
          Number(dataPoint.date),
          Number(dataPoint.indicator['aroon_down'])
        ]
      );
    }
