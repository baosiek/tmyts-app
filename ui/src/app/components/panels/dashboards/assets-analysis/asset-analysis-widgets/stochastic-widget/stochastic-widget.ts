...
@Component({
  selector: 'app-stochastic-widget',
  imports: [
    ...MATERIAL_IMPORTS,
    HighchartsChartDirective,
  ],
  templateUrl: './stochastic-widget.html',
  styleUrl: './stochastic-widget.scss'
})
export class StochasticWidget {
  ...
  getIndicatorData() {
    this.indicatorService.getStochasticIndicator([this.resolvedData().symbol])
      .pipe(
        catchError<any, any>(
          (error) => {
  ...
  dataIntoChartDataStructure(chartData: IndicatorModel[]) {
    ...
    for (const dataPoint of chartData) {
      ...
      this.k_line.push(
        [
          Number(dataPoint.date),
          Number(dataPoint.indicator['k_line'])
        ]
      );
      this.d_line.push(
        [
          Number(dataPoint.date),
          Number(dataPoint.indicator['d_line'])
        ]
      );
    }
