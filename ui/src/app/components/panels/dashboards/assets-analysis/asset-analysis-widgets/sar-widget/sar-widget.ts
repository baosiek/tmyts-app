...
@Component({
  ...
  templateUrl: './sar-widget.html',
  styleUrl: './sar-widget.scss'
})
export class SarWidget {
  ...
  getIndicatorData() {
    this.indicatorService.getPSARIndicator([this.resolvedData().symbol])
      .pipe(
        catchError<any, any>(
          (error) => {
  ...
  dataIntoChartDataStructure(chartData: IndicatorModel[]) {
    ...
    for (const dataPoint of chartData) {
      ...
      this.psar.push(
        [
          Number(dataPoint.date),
          Number(dataPoint.indicator['psar'])
        ]
      );
    }
      ...
      plotOptions: {
        ...
        scatter: {
          marker: {
            ...
            symbol: 'circle', // Shape of the dot (can be 'circle', 'square', 'diamond', 'triangle', 'triangle-down')
            states: {
              hover: {
                enabled: true,
